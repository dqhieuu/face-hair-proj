...
# FLAME2020: shape(100), exp(50), pose(15 = (jaw_pose(3), neck_pose(3), global_pose(3), eye_pose(6)))
# DECA 6 pose: (global_pose(3), jaw_pose(3)) (global pose in most cases isn't used)
...
class MyNet(nn.Module):
    ...
    def forward(self, my_emotion_params, shape_params=None):
        combined_input_params = torch.cat((shape_params, my_emotion_params),
                                          dim=1) if shape_params is not None else my_emotion_params
        # combined_input_params = my_emotion_params
        output = self.net(combined_input_params)
        # first 50 is exp, last 6 is pose
        exp_params, pose_params = output[:, :50], output[:, 50:]
        # zero out global pose params
        pose_params[:, 0:3] = 0
        print(exp_params, pose_params)
        return exp_params, pose_params

    @staticmethod
    def my_loss(pred, truth):
        pred_exp, pred_pose_global, pred_pose_jaw  = pred[:, 0:50], pred[:, 50:53], pred[:, 53:]
        truth_exp, truth_pose_global, truth_pose_jaw  = truth[:, 0:50], truth[:, 50:53], truth[:, 53:]
        mae = nn.L1Loss()
...
class MyNetWithoutShape(nn.Module):
    def __init__(self):
        super().__init__()
        self.mlp = nn.Sequential(
            nn.Linear(6, 128),
...
def get_emotion_model(use_flame_shape=False):
    if use_flame_shape:
        torch_model = MyNet().to(device)
        checkpoint = torch.load("decalib/emotion+shape_epoch=299-step=3600.ckpt", map_location=device)
    else:
        torch_model = MyNetWithoutShape().to(device)
        checkpoint = torch.load("decalib/emotion_only_epoch=299-step=3600.ckpt", map_location=device)
