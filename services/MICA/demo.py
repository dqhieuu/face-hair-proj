...
def process(args, app, image_size=224):
    ...
    for image_path in tqdm(image_paths):
        ...
        np.save(file, blob)
        cv2.imwrite(file + '.jpg', face_align.norm_crop(img,
                                                        landmark=face.kps, image_size=image_size))
...
def main(cfg, args):
    ...
    mica = util.find_model_using_name(
        model_dir='micalib.models', model_name=cfg.model.name)(cfg, device)
    ...
    with torch.no_grad():
        ...
        for path in tqdm(paths):
            ...
            dst.mkdir(parents=True, exist_ok=True)
            trimesh.Trimesh(vertices=mesh.cpu() * 1000.0, faces=faces,
                            process=False).export(f'{dst}/mesh.ply')  # save in millimeters
            trimesh.Trimesh(vertices=mesh.cpu() * 1000.0, faces=faces,
                            process=False).export(f'{dst}/mesh.obj')
...
def serve_prepare():
    global mica, app, faces, args, cfg
...
def my_process(img, image_size=224):
    dst = Path(args.a)
    dst.mkdir(parents=True, exist_ok=True)
    processes = []

    name = f'{int(time.time())}-{str(uuid.uuid4())}'
...
def serve_process(img, output_extension='obj'):
    with torch.no_grad():
        logger.info(f'Processing has started...')
        paths = my_process(img)
        for path in tqdm(paths):
            name = Path(path).stem
            images, arcface = to_batch(path)
            codedict = mica.encode(images, arcface)
            ...
            dst = Path(args.o, name)
            dst.mkdir(parents=True, exist_ok=True)
            mesh = trimesh.Trimesh(vertices=mesh.cpu(),
                                   faces=faces, process=False)
            if output_extension == 'obj':
                out = trimesh.exchange.obj.export_obj(mesh)
            elif output_extension == 'dae':
                out = trimesh.exchange.dae.export_collada(mesh)
...
def serve_process_string():
    ...
    output_format = content['output_extension']

    return serve_process(img, output_format)
